...
MODULE meshmod
    ...
    INTEGER, PARAMETER :: meshsize = 640               ! Size of mesh
	...
	INTEGER, PARAMETER :: bc = 3                       ! Switch to decide which BC to use at i=3/2 (1=2nd-up,2=2nd-cen,3=1st-up)
...
! set_ics (subroutine for initial conditions)
! set_bcs (subroutine for boundary condtions)
! rk2 (subroutine for time integration)
! calc_integral (subroutine for flux integral calculation)
! calc_analytic (subroutine to compute and output analytic sinusoidal CV-average solution)
! output (subroutine to output computed solution)

! flux (function to compute flux at a face)
! analytic (function to compute analytic values)
...
PROGRAM mech510_2  
    ...
    ! Validation for sinusoidal initial condition
	IF (ic == 1) THEN 
		tn = tmax
		CALL calc_analytic(tn)  ! Calculate and output analytic solution at time tmax
	END IF
   
    ! Output computed solution to .txt
    CALL output             
...
SUBROUTINE set_ics                     ! Subroutine to set initial conditions (CV-average values)
	...
	ELSE                               ! Linear (piecewise) initial condition
		mesh = 0.0D+0                  ! Initialize mesh with 0s
		
		DO i = 1,nint(1./delta_x)+1    ! Initialize (non-zero) interior values
			xb = i*delta_x
			xa = (i-1)*delta_x
			
            IF (i == nint(1./delta_x)+1) THEN
			    mesh(i) = -0.5         ! Average of -1 and 0
            ELSE
                ! CV-averages of T(x,0) from 0 to 1
...
FUNCTION flux(ind, meshn, Tw)                 ! Subroutine to calculate flux at face i+1/2
