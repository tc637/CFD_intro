...
MODULE meshmod
    ...
    INTEGER, PARAMETER :: meshsize = 500                          ! Size of mesh
    REAL*8, PARAMETER  :: cfl = 0.495                              ! CFL number (u*dt/dx)
    REAL*8, PARAMETER  :: xmax = 20.0D+0                           ! Maximum x-position
    REAL*8, PARAMETER  :: tmax = 8.0D+0                            ! Maximum time
    REAL*8, PARAMETER  :: u = 2.0D+0                               ! Wave speed
	INTEGER, PARAMETER :: ic = 2                                   ! Switch to decide which IC to use (1=sine, 2=lin)
...
SUBROUTINE calc_analytic(tn)          ! Calculate and output analytic solution at time tn for validation
    ...
    ! Set filename format
	fmt = "(A9,I0,A6,I0,A6,I0,A5,I0,A4,I1,A4)"  
    
    ! Set filename
    WRITE(fname, fmt) "analytic_",meshsize,"_xmax_",int(xmax),"_tmax_",int(tmax),"_cfl_",int(cfl*100),"_ic_",int(ic),".txt"
...
SUBROUTINE output  ! Output final solution for plotting
	...
	! Set filename format
	fmt = "(A8,I0,A6,I0,A6,I0,A5,I0,A4,I1,A4)"  
	
    ! Set filename
    WRITE(fname, fmt) "compute_",meshsize,"_xmax_",nint(xmax),"_tmax_",nint(tmax),"_cfl_",nint(cfl*1000.),"_ic_",int(ic),".txt"
