...
MODULE meshmod
    ...
    REAL*8, PARAMETER  :: Tw = 0.0                             ! Top wall velocity
    ...
    ! Time variables
	REAL*8, PARAMETER   :: dt = 0.1                           ! Time step variable
...
SUBROUTINE reach_ss(iss) ! Subroutine to check if solution has converged to steady-state
    ...
    REAL*8               :: maxdP, maxdu, maxdv  ! abs(max(dP)), abs(max(du)), abs(max(dv))
...
SUBROUTINE output_converge(oc)              ! Output convergence info for problem 3
    ...
    IF (oc == 1) THEN
        DO ind = 1,3
            ...
            ! Set filename
            WRITE(fnames(ind), fmt) "converg_",jmax,"_xmax_",nint(xmax),"_ymax_",nint(ymax),"_var_",varind,".txt"
