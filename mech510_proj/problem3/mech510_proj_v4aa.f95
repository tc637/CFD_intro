...
MODULE meshmod
    ...
    INTEGER, PARAMETER :: imax = 160                           ! Size of mesh (x-direction)
...
SUBROUTINE set_bcs(solmesh)  ! Subroutine to set ghost cells to enforce boundary conditions
    ...
    DO i=0,imax+1  ! Update bottom and top BCs
        ! Bottom wall (stationary) (actually top wall, but treated as bottom by symmetry)
        solmesh(0,i,1) = solmesh(1,i,1)  ! P (Neumann)
        solmesh(0,i,2) = -solmesh(1,i,2) ! u (Dirichlet)
        solmesh(0,i,3) = -solmesh(1,i,3) ! v (Dirchlet)
        
        ! Top wall (actually bottom wall, but treated as top by symmetry)
        ! Too difficult to handle flux integrals/Jacobians with decreasing indices going down
        ! Since symmetric in the vertical, will just plot upside-down
...
SUBROUTINE reach_ss(iss) ! Subroutine to check if solution has converged to steady-state
    ...
    REAL*8, DIMENSION(3) :: max_array            ! Array of maximum changes


    IF (problem == 3) THEN ! Use L2-norms to determine convergence
    ...
    ELSE ! Use Linf-norms to determine convergence
...
SUBROUTINE output_converge(oc)              ! Output convergence info for problem 3
	...
	! Set filename format
	fmt = "(A8,I0,A6,I0,A6,I0,A3,I0,A1,I0,A1,I0,A1,I0,A1,I0,A1,I0,A4)" 
    ...
    IF (oc == 1) THEN
        DO ind = 1,3
            ...
            ! Set filename
            WRITE(fnames(ind), fmt) "converg_",imax,"_xmax_",nint(xmax),"_ymax_",nint(ymax),"_p_",problem,"_", &
